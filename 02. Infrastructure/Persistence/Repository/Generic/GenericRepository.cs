...
public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : class
{
    ...
    public async Task<TEntity?> QuerySingleAsync(Func<IQueryable<TEntity>, IQueryable<TEntity>>? query,
    ...
    public async Task<List<T>> QueryListAsync<T>(Func<IQueryable<T>, IQueryable<T>>? query = null,
        bool asNoTracking = true) where T : class
    {
        try
        {
            var dbSet = query != null
                ? query(_unitOfWork.Context.Set<T>())
                : _unitOfWork.Context.Set<T>().AsQueryable();
    ...
    public async Task<List<IList>> MultipleResultsAsync(string query, List<Type> resultTypes,
