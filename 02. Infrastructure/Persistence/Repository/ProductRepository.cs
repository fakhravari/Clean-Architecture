...
namespace Persistence.Repository
{
    public class ProductRepository : GenericRepository<Product>, IProductRepository
    {
        ...
        public ProductRepository(IUnitOfWork<FakhravariDbContext> iUnitOfWork, ISerilogService logger) : base(
            iUnitOfWork, logger)
        {
        ...
        public async Task<List<GetListProductsDto>> GetListProducts(string IdCategory)
        {
            ...
            var product =
                await QueryListRawAsync<GetListProductsDto>($"EXEC SpGeneral.GetListProducts @IdCategory={IdCategory}");
            return product;
        }
        ...
        public async Task<List<GetListProductsDto>> GetListProducts1(string Title)
        {
            ...
            Func<IQueryable<Product>, IQueryable<Product>>? query = q =>
                q.Where(p => EF.Functions.Like(p.Title, $"%{Title}%"));
            var products = await QueryListAsync(query);

            Func<IQueryable<Product>, IQueryable<Product>>? query2 = q => q.Where(p => p.Title.Contains(Title));
            var products1 = await QueryListAsync(query2);
            ...
            for (var i = 0; i < 200; i++)
                lst.Add(new Product
                {
                    ...
                    Images = new List<Image>
                    {
                        ...
                        {
                            Id = Guid.NewGuid(),
                            ImageName = "https://fakhravari.ir/CDN/Site/Image/Mohammad_Hussein_Fakhravari_1.jpg"
                        },
                        new()
                        {
                            Id = Guid.NewGuid(),
        ...
        public async Task<object> MultipleResults()
        {
            ...
            var results = await MultipleResultsAsync(
                ...
                {
                    new("@IdCategory", SqlDbType.NVarChar) { Value = "0" }
                });
            ...
            Func<IQueryable<Product>, IQueryable<Product>>? query = q =>
                q.Where(p => EF.Functions.Like(p.Title, $"%{""}%"));
            var products33 = await QueryListAsync(query);
        ...
        public async Task<Guid> WorkManager(string data)
        {
            ...
            var sql =
                $" INSERT Security.Tokens(Id, Idconnection, IdPersonel, Token, DateTime, Ip, IsActive) SELECT '{Id}',CAST(GETDATE() AS DATE),1,@columnValue,GETDATE(),'--',1; SELECT 'ok'";
            var res = _unitOfWork.Context.Database.ExecuteSqlRaw(sql, columnValue);
