...
namespace WebApi.Services.ActionFilter
{
    ...
    public class ApiResultFilterAttribute : ActionFilterAttribute
    {
        public override void OnResultExecuting(ResultExecutingContext context)
        {
            ...
            else if (context.Result is BadRequestObjectResult bad)
            {
                ...
                if (bad.Value is SerializableError errors)
                {
                    var errorMessages = errors.SelectMany(p => (string[])p.Value).Distinct().ToList();
                    message = string.Join(" | ", errorMessages);
                }

                if (bad.Value is ValidationProblemDetails)
                {
                    var lst = new List<string>();

                    var _error = bad.Value as ValidationProblemDetails;
            ...
            else
            {
                var Response = new BaseResponse()
                {
                    ...
                    Data = "NotFound OnResultExecuting"
                };
                context.Result = new JsonResult(Response) { StatusCode = Response.StatusCode };
            }
        }
