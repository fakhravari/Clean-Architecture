    ...
    public class SwaggerApplyOperationFilter
    {
        public class ApplySummariesOperationFilter : IOperationFilter
        {
            public void Apply(OpenApiOperation operation, OperationFilterContext context)
            {
                ...
                if (IsGetAllAction())
                {
                    if (!Shared.Extentions.StringExtensions.HasValue(operation.Summary))
                ...
                else if (IsActionName("Read", "Get"))
                {
                    ...
                    if (!Shared.Extentions.StringExtensions.HasValue(operation.Parameters[0].Description))
        ...
        public class UnauthorizedResponsesOperationFilter : IOperationFilter
        {
            ...
            public void Apply(OpenApiOperation operation, OperationFilterContext context)
            {
                ...
                var hasAnonymous = Enumerable.Any<FilterDescriptor>(filters, p => p.Filter is Microsoft.AspNetCore.Mvc.Authorization.AllowAnonymousFilter) || Enumerable.Any<object>(metadta, p => p is AllowAnonymousAttribute);
                if (hasAnonymous) return;

                var hasAuthorize = Enumerable.Any<FilterDescriptor>(filters, p => p.Filter is Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter) || Enumerable.Any<object>(metadta, p => p is AuthorizeAttribute);
